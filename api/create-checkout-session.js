...
export default async function handler(req, res) {
  ...
  if (tier !== 'solo') {
    ...
    if (soloBinding.priceId && soloBinding.priceId === priceId) {
      ...
      return res.status(500).json({
        error: `Billing misconfiguration: ${binding.envName || priceEnvHint(tier, billingCycle)} points at the same Stripe Price as Solo (${soloBinding.envName}). Fix the Enterprise/Multiple Projects Price IDs in Vercel env.`,
      });
    }
  }
